...
const Header = ({ setView, user, onAuthRequest, onLogout, onWishlistClick, view }) => {
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 shadow-sm h-16 flex items-center justify-between px-4 md:px-8 lg:px-16 transition-colors duration-300 ${isHome ? 'bg-white' : 'bg-gradient-to-r from-[#1a3c8a] to-[#3b82f6]'}`}>
      ...
      <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setView('home')}>
        ...
        <span className="text-3xl font-serif font-medium tracking-tight ml-2">
          <span className={`${isHome ? 'text-[#333333]' : 'text-white/90 drop-shadow-sm mix-blend-overlay'}`}>Price</span>
        ...
        {user ? (
          ...
          <div className="flex items-center gap-4">
             ...
             <div className="hidden md:flex flex-col items-end mr-2">
               <span className={`text-sm font-bold ${isHome ? 'text-slate-900' : 'text-white'}`}>{user.displayName || 'User'}</span>
               <span className={`text-xs ${isHome ? 'text-slate-500' : 'text-blue-100'}`}>{user.email}</span>
             </div>
